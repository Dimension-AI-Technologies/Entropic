...
/// <summary>
/// Custom control that renders chat text with AvaloniaEdit for code blocks.
/// Prose gets plain TextBlocks; code blocks get read-only TextEditor with TextMate highlighting.
/// </summary>
public partial class AvaloniaEditContent : UserControl
{
    public static readonly StyledProperty<ChatMessage?> MessageProperty =
        AvaloniaProperty.Register<AvaloniaEditContent, ChatMessage?>(nameof(Message));

    public static readonly StyledProperty<string?> SearchTextProperty =
        AvaloniaProperty.Register<AvaloniaEditContent, string?>(nameof(SearchText));
    ...
    private void Rebuild()
    {
        ...
        while (i < segments.Length)
        {
            ...
            {
                var lang = segments[i].Trim();
                var code = segments[i + 1].Trim();
                if (!string.IsNullOrEmpty(code))
                    panel.Children.Add(BuildEditorBlock(code, lang));
                i += 2;
            }
            else
            {
    ...
    private Border BuildEditorBlock(string code, string lang)
    {
        var codeFg = CodeFg;
        var codeBg = CodeBg;
