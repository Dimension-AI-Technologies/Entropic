...
export function registerProjectsIpc(ipcMain: IpcMain, options: { projectsDir: string; logsDir: string }) {
  ipcMain.handle('get-todos', async () => {
    console.log('[IPC] get-todos called');
    const result = await loadTodosData(options.projectsDir, options.logsDir);
    if (result.success) {
