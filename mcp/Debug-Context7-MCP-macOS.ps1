...
Write-Host "=== Context7 MCP Server Diagnostic Tool (macOS/Linux) ===" -ForegroundColor Cyan
if ($Live) {
...
if ($OS -eq "Unix") {
    if (Test-Path "/System/Library/CoreServices/SystemVersion.plist") {
        ...
        $cacheDir = "$env:HOME/Library/Application Support/claude-cli-nodejs/Cache"
    } else {
        $OSType = "Linux"
        $npmPrefix = if (Get-Command npm -ErrorAction SilentlyContinue) { & npm config get prefix } else { "/usr/local" }
        $context7Path = "$npmPrefix/lib/node_modules/@upstash/context7-mcp"
...
try {
    if ($Live) {
        ...
        if ($completed) {
            ...
            Write-Host "  [ERROR] NPX execution timed out" -ForegroundColor Red
...
try {
    ...
    if ($nodeProcesses.Count -gt 0) {
        ...
        $recommendations += "Kill orphaned processes or restart system"
    ...
    if ($OSType -eq "Windows") {
        ...
        $allNodeProcesses = @{ Count = $allNodeCount }
    }
    Write-Host "  [INFO] Total node processes running: $($allNodeProcesses.Count)" -ForegroundColor Cyan
    
    if ($Verbose -and $OSType -eq "Windows") {
...
try {
    if ($OSType -eq "Windows") {
        ...
        $userContext = & whoami 2>$null
        if ($OSType -eq "macOS") {
            $osVersion = & sw_vers -productVersion 2>$null
            Write-Host "  [OK] OS: macOS $osVersion" -ForegroundColor Green
        } else {
            $osVersion = & lsb_release -d 2>$null | ForEach-Object { ($_ -split '\t')[1] }
            if (-not $osVersion) { $osVersion = "Unknown Linux" }
            Write-Host "  [OK] OS: $osVersion" -ForegroundColor Green
        }
    }
    ...
    if (Test-Path $context7Path) {
        if ($Live) {
            try {
                $testFile = Join-Path $context7Path "test.tmp"
            ...
            Write-Host "    Would create/delete test file in: $context7Path" -ForegroundColor Gray
        }
    }
}
catch {
...
if ($criticalIssues.Count -gt 0) {
    Write-Host "`n[CRITICAL] RECOMMENDATION: RESTART SYSTEM" -ForegroundColor Red
    Write-Host "The following issues require a system restart to resolve:" -ForegroundColor Red
    $criticalIssues | ForEach-Object { Write-Host "  - $_" -ForegroundColor Red }
    Write-Host "`nRestarting your system will:" -ForegroundColor Yellow
    ...
    if ($OSType -eq "Windows") {
        ...
        Write-Host "  - Clear system-level IPC/socket caches" -ForegroundColor Yellow
    }
    Write-Host "  - Force fresh MCP server connections" -ForegroundColor Yellow
    Write-Host "  - Reset process spawning state" -ForegroundColor Yellow
} else {
    Write-Host "`n[OK] No system restart required" -ForegroundColor Green
...
Write-Host "  .\Debug-Context7-MCP-macOS.ps1                    # Safe diagnostic only" -ForegroundColor Gray
Write-Host "  .\Debug-Context7-MCP-macOS.ps1 -Live              # Run actual tests" -ForegroundColor Gray
Write-Host "  .\Debug-Context7-MCP-macOS.ps1 -Live -KillOrphanedProcesses  # Full cleanup" -ForegroundColor Gray
Write-Host "  .\Debug-Context7-MCP-macOS.ps1 -ProjectPath '/Users/me/MyProject'   # Analyze specific project" -ForegroundColor Gray